...
// jQuery and luxon removed — D3 v7 + native DOM
...
export function drawLabeler(plottingApp) {
  ...
  plottingApp.main_margin = { top: 10, right: 120, bottom: 80, left: 90 },
    ...
    plottingApp.maindiv_width = Math.max(document.getElementById("maindiv").clientWidth || 800, 600),
  ...
  plottingApp.context.on("contextmenu", function (event) {
  ...
  if (!plottingApp.selectedSeries) {
    plottingApp.selectedSeries = (plottingApp.seriesList && plottingApp.seriesList[0]) || 'value';
  }
  if (!plottingApp.refSeries) {
    plottingApp.refSeries = (plottingApp.seriesList && plottingApp.seriesList[plottingApp.seriesList.length > 1 ? 1 : 0]) || 'value';
  }
  ...
  function init() {
    ...
    if (!plottingApp.csvData || plottingApp.csvData.length === 0) {
      ...
      var loader = document.querySelector(".loader"); if (loader) loader.style.display = "none";
      return;
    }
    ...
    // remove loading bar
    var loaderEl = document.querySelector(".loader"); if (loaderEl) loaderEl.style.display = "none";
  }
  ...
  function initPlot(defaultExtent) {
    ...
    // Set up wheel zoom on main chart (D3 v7: event as parameter)
    plottingApp.svg.on("wheel.zoom", function (event) {
      event.preventDefault();
      var scale = event.deltaY > 0 ? 2 : -2;  // Zoom out : Zoom in
  ...
  function updateYAxis() {
    ...
    p_editBtn.append("text")
      ...
      .attr("cursor", "pointer")
      .attr("fill", "currentColor")
      .on("click", function (event, d) { return updateMainY(plottingApp.selectedSeries); });
    ...
    if (plottingApp.refSeries != "" && plottingApp.selectedSeries != plottingApp.refSeries) {
      ...
      r_editBtn.append("text")
        ...
        .attr("cursor", "pointer")
        .attr("fill", "currentColor")
        .on("click", function (event, d) { return updateMainY(plottingApp.refSeries); });
    }
  }
  ...
  function updateMain() {
    ...
    plottingApp.main.selectAll(".point")
      ...
      .on("click", function (event, point) {
        //allow clicking on single points (D3 v7: event, datum)
  ...
  function brushedMain() {
    ...
    // Trigger Vue update when brushing ends — via callbacks (no window.vueApp)
    if (plottingApp.selection) {
      if (typeof plottingApp.onChartSelection === 'function') {
        plottingApp.onChartSelection(plottingApp.selection.start, plottingApp.selection.end);
      } else if (typeof plottingApp.onSelectionRange === 'function') {
  ...
  function toggleHoverinfo(b) {
    if (b) {
      // enable mouseover/mouseout hoverinfo (D3 v7: event, datum)
      plottingApp.main.selectAll(".point")
  ...
  function updateHoverinfo(time, val, label) {
    ...
    if (time === "" && val === "" && label == "") {
      ...
      if (typeof plottingApp.onHoverUpdate === 'function') plottingApp.onHoverUpdate();
    } else {
  ...
  function updateMainY(axis) {
    ...
    if (typeof plottingApp.onEditAxis === 'function') plottingApp.onEditAxis();
  }
  ...
  // Series change via Vue callback (jQuery selectors removed)
  plottingApp.changeSeries = function (series) {
    plottingApp.selectedSeries = series;
    plottingApp.data = plottingApp.allData.filter(d => d.series == plottingApp.selectedSeries);
    replot();
  };

  plottingApp.changeReference = function (series) {
    setReference(series);
    replot();
  };

  plottingApp.changeLabelSelect = function (label) {
    plottingApp.selectedLabel = label;
  };

  plottingApp.clearAllLabels = function () {
    plottingApp.quadtree.visit(function (node, quad_xmin, quad_ymin, quad_xmax, quad_ymax) {
      if (!node.length) {
        do {
          node.data.label = '';
  ...
  plottingApp.triggerReplot = function () {
    replot();
  };

  plottingApp.triggerRecolor = function () {
    updateSelection();
  };

  plottingApp.exportCSV = function () {
    var csvContent = plottingApp.headerStr + "\n";

    plottingApp.allData.forEach(function (dataArray) {
      var date = dataArray.actual_time;
  ...
  // Keyboard navigation (D3 v7: event as parameter)
  d3.select(window).on("keydown.labeler", function (event) {
    plottingApp.shiftKey = event.shiftKey;
    ...
    if (code == 38) {
      ...
      if (plottingApp.hoverTimer && plottingApp.hoverinfo.label) {
        if (plottingApp.selectedLabel != plottingApp.hoverinfo.label) {
          ...
          if (typeof plottingApp.onLabelChange === 'function') plottingApp.onLabelChange();
        }
      }
    }
  });

  d3.select(window).on("keyup.labeler", function (event) {
    plottingApp.shiftKey = event.shiftKey;
  });
}
