...
| 优先级 | 任务 | 当前状态 | 验收标准 |
|--------|------|----------|----------|
| P0 | 修复推理任务状态枚举不一致 | 待开始 | 推理任务状态流转统一为 schema 中的合法枚举值 |
| P0 | 补齐 `convert_to_annotation_format` 导出链路 | 待开始 | `core/tasks.py` 与 `/api/v1/inference/export-to-annotation` 均可正常导出 |
| P0 | 恢复 Annotator 鉴权 | 待开始 | 默认需认证；仅开发模式可显式开启绕过 |
| P1 | 数据资产管理 API 化 | 待开始 | WebUI 不再直连 DB；资产功能通过 `/api/v1/assets` 完成 |
| P1 | 数据库迁移机制 | 待开始 | 新旧库可升级；不依赖仅 `create_all` |
| P1 | 统一任务执行模型 | 待开始 | 训练/推理/采集任务状态、日志、失败语义一致 |
| P2 | 版本与文档口径统一 | 待开始 | `settings.APP_VERSION` 与发布文档一致 |
| P2 | 最小回归测试集 | 待开始 | 覆盖索引写入、审核、导出、评估四条主链路 |
