...
| Week 7 | 2026-03-02 ~ 2026-03-08（截至 3/2） | 安全与稳定性加固 | 训练任务生命周期修复、采集链路安全加固、JWT/CORS 基线校验、IoTDB 凭据默认值清理、新增回归测试 |
...
*   新增安全与任务生命周期相关测试用例，为后续合并提供回归保障。

---

### 2月7日 ~ 3月2日（补充日报）
> 说明：按“有开发内容记事项、无开发内容记‘无’”补齐；记录来源为 Git 提交记录与当前分支开发工作树（截至 2026-03-02）。
...
| 2026-02-28 | 无 |
| 2026-03-01 | 无 |
| 2026-03-02 | 安全与稳定性加固（开发中）：训练任务生命周期修复（避免“提前完成”）与取消联动、采集配置脱敏、CSV 预览路径安全校验、JWT/CORS 安全基线校验、IoTDB 凭据默认值清理；新增安全与生命周期回归测试。 |

---

### 3月2日（安全与稳定性加固，开发中）
> **阶段目标**: 修复运行稳定性与安全基线薄弱项，降低线上风险。

*   **任务生命周期修复（训练）**:
    *   修复 Celery 训练任务“提前完成”问题，改为等待训练子进程真实结束后再落库状态。
    *   补齐取消联动能力（DB 状态驱动子进程中断），修正任务可控性。
*   **采集链路安全加固**:
    *   采集任务配置落库脱敏（密码不明文持久化）。
    *   CSV 预览新增路径边界校验，阻断目录穿越风险。
*   **配置安全基线**:
    *   JWT 与 CORS 改为强校验（弱密钥/`*` 白名单启动失败）。
    *   清理 IoTDB 默认 root/root 凭据，改为显式配置。
*   **回归验证**:
    *   新增/更新对应测试（含 `tests/test_security_baseline.py`、`tests/test_training_task_lifecycle.py`）。
