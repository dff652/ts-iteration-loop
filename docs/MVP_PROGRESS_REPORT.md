...
**版本截止**: v0.3.3
**时间范围**: 2026-01-19 至 2026-02-03

---

## 🚀 1. 项目整体进度汇报 (MVP 完成)

本项目已完成 MVP (Minimum Viable Product) 开发，并快速迭代至 v0.3.3 版本，成功打通了时序数据从采集到多模态检测的全链路闭环。
...
### 2月3日 (v0.3.3 - 索引与审核闭环)
> **阶段目标**: 引入推理置信度产物与 DB 索引，落地候选/审核队列，形成可筛选的标注闭环。

*   **推理置信度产物**:
    *   推理结束自动生成 `metrics.json` / `segments.json`，并写入 DB 索引。
    *   支持段级指标入库与摘要评分。
*   **候选/审核队列**:
    *   候选队列新增置信度筛选、TopK/低置信度/随机抽样。
    *   审核队列支持状态流转、批量审核与进度统计。
*   **训练导出**:
    *   训练数据导出仅保留 `approved` 标注（单用户 MVP）。
    *   支持历史 CSV 批量回填索引脚本。
